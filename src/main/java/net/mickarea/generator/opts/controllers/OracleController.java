...
This file "OracleController.java" is part of project "generator" , which is belong to Michael Pang (It's Me).
...
public class OracleController extends AbstractController {
	...
	public SqlResult testConnection(CommandArguments cArgs) {
		...
		try {
			...
			//执行数据库查询
			SimpleDBData sdb = MyDBUtil.mySqlQuery(mana.getDataPoolByName(cArgs.getPoolName()), "select 1 from dual", null);
			//根据状态处理
	...
	public SqlResult searchDictInfo(CommandArguments cArgs) {
		...
		try {
			...
			mana = DatabasePoolManager.getInstance(cArgs);
			//执行数据库查询
			String preSql = "select a.table_name from all_tables a where a.owner=? order by a.table_name";
	...
	protected GenResult genMyResults(String tabName, String schemaName, DataSource ds, String fileDir, String charSet) {
		...
		try {
			//准备数据字典查询语句
			String preSql = "select a.column_name,\r\n" + 
					"       a.column_id,\r\n" + 
					"       a.data_type,\r\n" + 
					"       a.data_type column_type,\r\n" + 
					"       nvl((select 'PRI'\r\n" + 
					"          from all_ind_columns l , all_indexes s\r\n" + 
					"         where l.table_name = s.table_name\r\n" + 
					"           and l.table_owner = s.table_owner\r\n" + 
					"           and s.uniqueness = 'UNIQUE'\r\n" + 
					"           and l.table_name=a.table_name\r\n" + 
					"           and l.table_owner=a.owner\r\n" + 
					"           and l.column_name = a.column_name),' ') column_key,\r\n" + 
					"       nvl(null,' ') extra,\r\n" + 
					"       nvl((select c.comments from all_col_comments c \r\n" + 
					"         where c.owner=a.owner \r\n" + 
					"           and c.table_name=a.table_name \r\n" + 
					"           and c.column_name=a.column_name),' ') as column_comment\r\n" + 
